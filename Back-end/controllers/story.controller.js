...
export const getBookmarkedSlides = async (req, res) => {
  try {
      ...
      const user = await User.findById(userId, 'bookmarks'); // Fetch only the bookmarks field
      ...
      res.status(200).json({
          message: "Bookmarked slide IDs fetched successfully.",
          bookmarks: user.bookmarks, // Only return the slide IDs
      });
  } catch (error) {
